...
def generate_cg_array_sparse():
        ...
        for m1 in m_vals[j1]:
            ...
            for j2 in j_vals:
                for m2 in m_vals[j2]:
                    ...
                    j3_vals = range(abs( j1 - j2 ), j1 + j2 + 2, 2)
                    for j3 in j3_vals:

                        j1_idx = (j1 + 1)//2 - 1        # [1, 3, 5] -> [0, 1, 2]
                        m1_idx = (m1 + j1)//2           # [-5, -3, -1, 1, 3, 5] -> [0, 1, 2, 3, 4, 5], [-1, 1] -> [0, 1]
                        j2_idx = (j2 + 1)//2 - 1
                        m2_idx = (m2 + j2)//2
                        j3_idx = (j3 - j3_vals[0])//2   # [0,  2,  4,  6,  8, 10] -> [0, 1, 2, 3, 4, 5], [2, 4, 6, 8] -> [0, 1, 2, 3]
                        ...
                        calculated_flat_idx = ((((j1_idx*len(m_vals[j1]) + m1_idx)*3 + j2_idx)*6 + len(m_vals[j2]))*len(j3_vals) + j3_idx)    # Indexing a 1D array as if it was a 5D array.
                        arr_flat.append(float(cg.doit()))
    ...
    msg = ", "
    with open("cg_flat_sparse_arr.txt", "w") as outfile:
        for i in range(len(arr_flat)):
