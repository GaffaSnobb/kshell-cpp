...
namespace bittools
{
inline uint16_t reset_bit_and_count_swaps(uint64_t& state, const uint16_t bit_to_reset)
{
    const uint64_t mask = (1ULL << bit_to_reset) - 1;           // Create a mask for all bits before the bit_to_reset.
    const int32_t count = __builtin_popcountll(state & mask);   // Count set bits before the bit_to_reset.
    state &= ~(1ULL << bit_to_reset);                           // Reset the bit at bit_to_reset.
    return count;
}

inline uint16_t set_bit_and_count_swaps(uint64_t& state, const uint16_t bit_to_set)
{
    const uint64_t mask = (1ULL << bit_to_set) - 1;             // Create a mask for all bits before the bit_to_set.
    const int32_t count = __builtin_popcountll(state & mask);   // Count set bits before the bit_to_set.
    state |= (1ULL << bit_to_set);                              // Set the bit at bit_to_set.
    return count;
}

inline uint16_t set_bit_and_count_swaps(std::bitset<n_bits_bitset>& state, const uint16_t bit_to_set)
{
    /*
    ...
    Performance notes
    -----------------
    (1): for (uint16_t i = 0; i < bit_to_set; i++) if (state.test(i)) count++;
    (2): for (uint16_t i = 0; i < bit_to_set; i++) if (state[i]) count++;
    (3): for (uint16_t i = 0; i < bit_to_set; i++) count += state[i];    
    ...
    */
    uint16_t count = 0;
    // for (uint16_t i = 0; i < bit_to_set; i++) if (state.test(i)) count++;
    // for (uint16_t i = 0; i < bit_to_set; i++) if (state[i]) count++;
    for (uint16_t i = 0; i < bit_to_set; i++) count += state[i];
    // state.set(bit_to_set);
    state[bit_to_set] = 1;
    return count;
}

inline uint16_t reset_bit_and_count_swaps(std::bitset<n_bits_bitset>& state, const uint16_t bit_to_reset)
{
    /*
    ...
    */
    uint16_t count = 0;
    // for (uint16_t i = 0; i < bit_to_reset; i++) if (state.test(i)) count++;
    // for (uint16_t i = 0; i < bit_to_reset; i++) if (state[i]) count++;
    for (uint16_t i = 0; i < bit_to_reset; i++) count += state[i];
    // state.reset(bit_to_reset);
    state[bit_to_reset] = 0;
    return count;
}

inline void set_bit(uint64_t &state, const uint16_t bit_to_set)
{
    /*
    Sets a specific bit in an uint64_t number.
    */
    state |= (1ULL << bit_to_set);
}

inline bool is_bit_set(const uint64_t state, const uint16_t bit_to_check)
